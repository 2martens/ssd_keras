...
"""
A Keras port of the original Caffe SSD300 network. Modified to include two
...
from twomartens.masterthesis.ssd_keras.keras_layers import keras_layer_DecodeDetections
from twomartens.masterthesis.ssd_keras.keras_layers import keras_layer_DecodeDetectionsFast
...
def ssd_300_dropout(image_size: Tuple[int, int, int],
                    ...
                    two_boxes_for_ar1: Optional[bool] = True,
                    ...
                    clip_boxes: Optional[bool] = False,
                    variances: Optional[Sequence[float]] = None,
                    coords: Optional[str] = 'centroids',
                    ...
                    confidence_thresh: Optional[float] = 0.01,
    ...
    Arguments:
        ...
        dropout_rate: The rate for the dropout layers. A float between 0 and 1 that specifies how many units are
            dropped.
        ...
        offsets (list, optional): `None` or a list with as many elements as there are predictor layers. The elements
            ...
            should be
            as a fraction of `steps`. The last bit is important: The offsets are not absolute pixel values,
            but fractions
            of the step size specified in the `steps` argument. If the list contains floats, then that value will
            be used for both spatial dimensions. If the list contains tuples of two floats, then they represent
            `(vertical_offset, horizontal_offset)`. If no offsets are provided, then they will default to 0.5 of the
            step size.
    ...
    conv1_1 = tf.keras.layers.Conv2D(64, (3, 3), activation='relu', padding='same', kernel_initializer='he_normal',
                                     kernel_regularizer=tf.keras.regularizers.l2(l2_reg), name='conv1_1')(x1)
    ...
    pool5 = tf.keras.layers.MaxPooling2D(pool_size=(3, 3), strides=(1, 1), padding='same', name='pool5')(conv5_3)
    
    fc6 = tf.keras.layers.Conv2D(1024, (3, 3), dilation_rate=(6, 6), activation='relu', padding='same',
                                 kernel_initializer='he_normal', kernel_regularizer=tf.keras.regularizers.l2(l2_reg),
                                 name='fc6')(pool5)
    dropout_1 = tf.keras.layers.Dropout(rate=dropout_rate)(fc6)
    ...
    dropout_2 = tf.keras.layers.Dropout(rate=dropout_rate)(fc7)
    ...
    conv6_1 = tf.keras.layers.ZeroPadding2D(padding=((1, 1), (1, 1)), name='conv6_padding')(conv6_1)
    conv6_2 = tf.keras.layers.Conv2D(512, (3, 3), strides=(2, 2), activation='relu', padding='valid',
                                     kernel_initializer='he_normal',
                                     kernel_regularizer=tf.keras.regularizers.l2(l2_reg), name='conv6_2')(conv6_1)
    
    conv7_1 = tf.keras.layers.Conv2D(128, (1, 1), activation='relu', padding='same', kernel_initializer='he_normal',
                                     kernel_regularizer=tf.keras.regularizers.l2(l2_reg), name='conv7_1')(conv6_2)
    conv7_1 = tf.keras.layers.ZeroPadding2D(padding=((1, 1), (1, 1)), name='conv7_padding')(conv7_1)
    conv7_2 = tf.keras.layers.Conv2D(256, (3, 3), strides=(2, 2), activation='relu', padding='valid',
                                     kernel_initializer='he_normal',
                                     kernel_regularizer=tf.keras.regularizers.l2(l2_reg), name='conv7_2')(conv7_1)
    
    conv8_1 = tf.keras.layers.Conv2D(128, (1, 1), activation='relu', padding='same', kernel_initializer='he_normal',
                                     kernel_regularizer=tf.keras.regularizers.l2(l2_reg), name='conv8_1')(conv7_2)
    conv8_2 = tf.keras.layers.Conv2D(256, (3, 3), strides=(1, 1), activation='relu', padding='valid',
                                     kernel_initializer='he_normal',
                                     kernel_regularizer=tf.keras.regularizers.l2(l2_reg), name='conv8_2')(conv8_1)
    
    conv9_1 = tf.keras.layers.Conv2D(128, (1, 1), activation='relu', padding='same', kernel_initializer='he_normal',
                                     kernel_regularizer=tf.keras.regularizers.l2(l2_reg), name='conv9_1')(conv8_2)
    conv9_2 = tf.keras.layers.Conv2D(256, (3, 3), strides=(1, 1), activation='relu', padding='valid',
                                     kernel_initializer='he_normal',
                                     kernel_regularizer=tf.keras.regularizers.l2(l2_reg), name='conv9_2')(conv9_1)
    
    # Feed conv4_3 into the L2 normalization layer
    conv4_3_norm = keras_layer_L2Normalization.L2Normalization(gamma_init=20, name='conv4_3_norm')(conv4_3)
    ...
    conv7_2_mbox_conf = tf.keras.layers.Conv2D(n_boxes[3] * n_classes, (3, 3), padding='same',
                                               kernel_initializer='he_normal',
                                               kernel_regularizer=tf.keras.regularizers.l2(l2_reg),
                                               name='conv7_2_mbox_conf')(conv7_2)
    conv8_2_mbox_conf = tf.keras.layers.Conv2D(n_boxes[4] * n_classes, (3, 3), padding='same',
                                               kernel_initializer='he_normal',
                                               kernel_regularizer=tf.keras.regularizers.l2(l2_reg),
                                               name='conv8_2_mbox_conf')(conv8_2)
    conv9_2_mbox_conf = tf.keras.layers.Conv2D(n_boxes[5] * n_classes, (3, 3), padding='same',
                                               kernel_initializer='he_normal',
                                               kernel_regularizer=tf.keras.regularizers.l2(l2_reg),
                                               name='conv9_2_mbox_conf')(conv9_2)
    # We predict 4 box coordinates for each box, hence the localization predictors have depth `n_boxes * 4`
    # Output shape of the localization layers: `(batch, height, width, n_boxes * 4)`
    conv4_3_norm_mbox_loc = tf.keras.layers.Conv2D(n_boxes[0] * 4, (3, 3), padding='same',
                                                   kernel_initializer='he_normal',
                                                   kernel_regularizer=tf.keras.regularizers.l2(l2_reg),
    ...
    fc7_mbox_loc = tf.keras.layers.Conv2D(n_boxes[1] * 4, (3, 3), padding='same', kernel_initializer='he_normal',
                                          kernel_regularizer=tf.keras.regularizers.l2(l2_reg), name='fc7_mbox_loc')(fc7)
    conv6_2_mbox_loc = tf.keras.layers.Conv2D(n_boxes[2] * 4, (3, 3), padding='same', kernel_initializer='he_normal',
                                              kernel_regularizer=tf.keras.regularizers.l2(l2_reg),
                                              name='conv6_2_mbox_loc')(conv6_2)
    conv7_2_mbox_loc = tf.keras.layers.Conv2D(n_boxes[3] * 4, (3, 3), padding='same', kernel_initializer='he_normal',
                                              kernel_regularizer=tf.keras.regularizers.l2(l2_reg),
                                              name='conv7_2_mbox_loc')(conv7_2)
    conv8_2_mbox_loc = tf.keras.layers.Conv2D(n_boxes[4] * 4, (3, 3), padding='same', kernel_initializer='he_normal',
                                              kernel_regularizer=tf.keras.regularizers.l2(l2_reg),
                                              name='conv8_2_mbox_loc')(conv8_2)
    conv9_2_mbox_loc = tf.keras.layers.Conv2D(n_boxes[5] * 4, (3, 3), padding='same', kernel_initializer='he_normal',
                                              kernel_regularizer=tf.keras.regularizers.l2(l2_reg),
                                              name='conv9_2_mbox_loc')(conv9_2)
    
    # Generate the anchor boxes (called "priors" in the original Caffe/C++ implementation, so I'll keep their layer
    # names)
    
    # Output shape of anchors: `(batch, height, width, n_boxes, 8)`
    conv4_3_norm_mbox_priorbox = keras_layer_AnchorBoxes.AnchorBoxes(img_height, img_width, this_scale=scales[0],
                                                                     next_scale=scales[1],
    ...
    fc7_mbox_priorbox = keras_layer_AnchorBoxes.AnchorBoxes(img_height, img_width, this_scale=scales[1],
    ...
    conv6_2_mbox_priorbox = keras_layer_AnchorBoxes.AnchorBoxes(img_height, img_width, this_scale=scales[2],
    ...
    conv7_2_mbox_priorbox = keras_layer_AnchorBoxes.AnchorBoxes(img_height, img_width, this_scale=scales[3],
    ...
    conv8_2_mbox_priorbox = keras_layer_AnchorBoxes.AnchorBoxes(img_height, img_width, this_scale=scales[4],
    ...
    conv9_2_mbox_priorbox = keras_layer_AnchorBoxes.AnchorBoxes(img_height, img_width, this_scale=scales[5],
    ...
    conv4_3_norm_mbox_conf_reshape = tf.keras.layers.Reshape((-1, n_classes), name='conv4_3_norm_mbox_conf_reshape')(
        conv4_3_norm_mbox_conf)
    fc7_mbox_conf_reshape = tf.keras.layers.Reshape((-1, n_classes), name='fc7_mbox_conf_reshape')(fc7_mbox_conf)
    conv6_2_mbox_conf_reshape = tf.keras.layers.Reshape((-1, n_classes), name='conv6_2_mbox_conf_reshape')(
        conv6_2_mbox_conf)
    conv7_2_mbox_conf_reshape = tf.keras.layers.Reshape((-1, n_classes), name='conv7_2_mbox_conf_reshape')(
        conv7_2_mbox_conf)
    conv8_2_mbox_conf_reshape = tf.keras.layers.Reshape((-1, n_classes), name='conv8_2_mbox_conf_reshape')(
        conv8_2_mbox_conf)
    conv9_2_mbox_conf_reshape = tf.keras.layers.Reshape((-1, n_classes), name='conv9_2_mbox_conf_reshape')(
        conv9_2_mbox_conf)
    ...
    conv4_3_norm_mbox_priorbox_reshape = tf.keras.layers.Reshape((-1, 8), name='conv4_3_norm_mbox_priorbox_reshape')(
        conv4_3_norm_mbox_priorbox)
    fc7_mbox_priorbox_reshape = tf.keras.layers.Reshape((-1, 8), name='fc7_mbox_priorbox_reshape')(fc7_mbox_priorbox)
    ...
    mbox_conf = tf.keras.layers.Concatenate(axis=1, name='mbox_conf')([conv4_3_norm_mbox_conf_reshape,
    ...
    mbox_loc = tf.keras.layers.Concatenate(axis=1, name='mbox_loc')([conv4_3_norm_mbox_loc_reshape,
    ...
    mbox_priorbox = tf.keras.layers.Concatenate(axis=1, name='mbox_priorbox')([conv4_3_norm_mbox_priorbox_reshape,
    ...
    mbox_conf_softmax = tf.keras.layers.Activation('softmax', name='mbox_conf_softmax')(mbox_conf)
    
    # Concatenate the class and box predictions and the anchors to one large predictions vector
    # Output shape of `predictions`: (batch, n_boxes_total, n_classes + 4 + 8)
    predictions = tf.keras.layers.Concatenate(axis=2, name='predictions')([mbox_conf_softmax, mbox_loc, mbox_priorbox])
    
    if mode == 'training':
        model = tf.keras.Model(inputs=x, outputs=predictions)
    elif mode == 'inference':
        decoded_predictions = keras_layer_DecodeDetections.DecodeDetections(confidence_thresh=confidence_thresh,
        ...
        model = tf.keras.Model(inputs=x, outputs=decoded_predictions)
    elif mode == 'inference_fast':
        decoded_predictions = keras_layer_DecodeDetectionsFast. \
            DecodeDetectionsFast(confidence_thresh=confidence_thresh,
                                 iou_threshold=iou_threshold,
                                 top_k=top_k,
                                 nms_max_output_size=nms_max_output_size,
                                 coords=coords,
    ...
    if return_predictor_sizes:
        predictor_sizes = np.array([conv4_3_norm_mbox_conf.int_shape()[1:3],
                                    fc7_mbox_conf.int_shape()[1:3],
                                    conv6_2_mbox_conf.int_shape()[1:3],
                                    conv7_2_mbox_conf.int_shape()[1:3],
                                    conv8_2_mbox_conf.int_shape()[1:3],
                                    conv9_2_mbox_conf.int_shape()[1:3]])
