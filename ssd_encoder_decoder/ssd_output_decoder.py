...
def greedy_nms(y_pred_decoded, iou_threshold=0.45, coords='corners', border_pixels='half'):
    ...
    for batch_item in y_pred_decoded:  # For the labels of each batch item...
        ...
        while boxes_left.shape[0] > 0:  # While there are still boxes left to compare...
            ...
            similarities = bounding_box_utils.iou(boxes_left[:, 2:], maximum_box[2:], coords=coords,
                                                  mode='element-wise',
                                                  border_pixels=border_pixels)  # ...compare (IoU) the other left
            # over boxes to the maximum box...
            boxes_left = boxes_left[
                similarities <= iou_threshold]  # ...so that we can remove the ones that overlap too much with the
            # maximum box
...
def _greedy_nms(predictions, iou_threshold=0.45, coords='corners', border_pixels='half'):
    ...
    while boxes_left.shape[0] > 0:  # While there are still boxes left to compare...
        ...
        if boxes_left.shape[0] == 0:
            break  # If there are no boxes left after this step, break. Otherwise...
        similarities = bounding_box_utils.iou(boxes_left[:, 1:], maximum_box[1:], coords=coords, mode='element-wise',
...
def decode_detections(y_pred,
    ...
    non-maximum suppression globally instead of per class, see `decode_detections_fast()` below.
    ...
    Arguments:
        ...
        entropy_thresh (float, optional): A float describing the upper limit for the entropy across all classes.
            Predictions with higher entropy will be filtered out.
    ...
    n_classes = y_pred_decoded_raw.shape[
                    -1] - 5  # The number of classes is the length of the last axis minus the four box coordinates and
    # one placeholder
    ...
    for batch_item in y_pred_decoded_raw:  # `batch_item` has shape `[n_boxes, n_classes + 5 coords]`
        ...
        for class_id in range(1, n_classes):  # For each class except the background class (which has class ID 0)...
            single_class = batch_item[:, [class_id, -5, -4, -3,
                                          -2]]  # ...keep only the confidences for that class, making this an array
            # of shape `[n_boxes, 5]` and...
            threshold_met = single_class[single_class[:,
                                         0] > confidence_thresh]  # ...keep only those boxes with a confidence above
            # the set threshold.
            if threshold_met.shape[0] > 0:  # If any boxes made the threshold...
                ...
                maxima_output = np.zeros((maxima.shape[0], maxima.shape[
                    1] + 1))  # Expand the last dimension by one element to have room for the class ID. This is now
                # an arrray of shape `[n_boxes, 6]`
        ...
        if pred:  # If there are any predictions left after confidence-thresholding...
            ...
            if top_k != 'all' and pred.shape[0] > top_k:  # If we have more than `top_k` results left at this point,
        ...
        y_pred_decoded.append(
            pred)  # ...and now that we're done, append the array of final predictions for this batch item to the
        # output list
...
def decode_detections_dropout(y_pred,
    ...
    for i in range(y_pred_decoded_raw.shape[0]):  # `batch_item` has shape `[n_boxes, n_classes + 5 coords]`
        ...
        pred = pred[i, np.nonzero(np.argmax(pred[i, :, :-5], axis=-1))]
...
def decode_detections_fast(y_pred: np.ndarray,
    ...
    for batch_item in y_pred_converted:  # For each image in the batch...
        ...
        # ...then filter out those positive boxes for which the entropy is too high and after that,...
        if entropy_thresh:
            boxes = boxes[boxes[:, 2] <= entropy_thresh]
        else:
            # alternatively...
            # ...filter out the boxes for which the prediction confidence is too low and after that,...
            boxes = boxes[boxes[:,
                          1] >= confidence_thresh]
        ...
        y_pred_decoded.append(
            boxes)  # ...and now that we're done, append the array of final predictions for this batch item to the
        # output list
...
def decode_detections_debug(y_pred,
    ...
    Returns:
        ...
        a non-background class for the respective image in the format `[box_id, class_id, confidence, xmin, ymin,
        xmax, ymax]`.
    """
    if normalize_coords and ((img_height is None) or (img_width is None)):
        raise ValueError(
    ...
    y_pred_decoded_raw = np.copy(y_pred[:, :,
                                 :-8])  # Slice out the classes and the four offsets, throw away the anchor
    # coordinates and variances, resulting in a tensor of shape `[batch, n_boxes, n_classes + 4 coordinates]`
    
    if input_coords == 'centroids':
        if variance_encoded_in_target:
            # Decode the predicted box center x and y coordinates.
            y_pred_decoded_raw[:, :, [-4, -3]] = y_pred_decoded_raw[:, :, [-4, -3]] * y_pred[:, :, [-6, -5]] + \
                                                 y_pred[:, :, [-8, -7]]
            ...
            # Decode the predicted box center x and y coordinates.
            y_pred_decoded_raw[:, :, [-4, -3]] = y_pred_decoded_raw[:, :, [-4, -3]] * y_pred[:, :, [-6, -5]] * \
                                                 y_pred[:, :, [-4, -3]] + y_pred[:, :, [-8, -7]]
            # Decode the predicted box width and heigt.
            y_pred_decoded_raw[:, :, [-2, -1]] = np.exp(
                y_pred_decoded_raw[:, :, [-2, -1]] * y_pred[:, :, [-2, -1]]) * y_pred[:, :, [-6, -5]]
        y_pred_decoded_raw = bounding_box_utils.convert_coordinates(y_pred_decoded_raw, start_index=-4,
                                                                    conversion='centroids2corners')
    elif input_coords == 'minmax':
        y_pred_decoded_raw[:, :, -4:] *= y_pred[:, :, -4:]  # delta(pred) / size(anchor) / variance * variance ==
        # delta(pred) / size(anchor) for all four coordinates, where 'size' refers to w or h, respectively
    ...
    n_classes = y_pred_decoded_raw.shape[
                    -1] - 5  # The number of classes is the length of the last axis minus the four box coordinates
    # and minus the index
    ...
    for batch_item in y_pred_decoded_raw:  # `batch_item` has shape `[n_boxes, n_classes + 4 coords]`
        ...
        for class_id in range(1, n_classes):  # For each class except the background class (which has class ID 0)...
            single_class = batch_item[:, [0, class_id + 1, -4, -3, -2,
                                          -1]]  # ...keep only the confidences for that class, making this an array
            # of shape `[n_boxes, 6]` and...
            threshold_met = single_class[single_class[:,
                                         1] > confidence_thresh]  # ...keep only those boxes with a confidence above
            # the set threshold.
            if threshold_met.shape[0] > 0:  # If any boxes made the threshold...
        ...
        # Once we're through with all classes, keep only the `top_k` maxima with the highest scores
        pred = np.concatenate(pred, axis=0)
        if pred.shape[0] > top_k:  # If we have more than `top_k` results left at this point,
            # otherwise there is nothing to filter,...
            top_k_indices = np.argpartition(pred[:, 2], kth=pred.shape[0] - top_k, axis=0)[
                            pred.shape[0] - top_k:]  # ...get the indices of the `top_k` highest-score maxima...
            pred = pred[top_k_indices]  # ...and keep only those entries of `pred`...
        y_pred_decoded.append(
            pred)  # ...and now that we're done,
        # append the array of final predictions for this batch item to the output list
...
def _greedy_nms_debug(predictions, iou_threshold=0.45, coords='corners', border_pixels='half'):
    ...
    while boxes_left.shape[0] > 0:  # While there are still boxes left to compare...
        ...
        # boxes to the maximum box...
        boxes_left = boxes_left[
            similarities <= iou_threshold]  # ...so that we can remove the ones that overlap too much
...
def get_pred_layers(y_pred_decoded: np.ndarray, num_boxes_per_pred_layer: Sequence[int]):
    """
    ...
    for batch_item in y_pred_decoded:
        ...
        for prediction in batch_item:
            if (prediction[0] < 0) or (prediction[0] >= cum_boxes_per_pred_layer[-1]):
                raise ValueError("Box index is out of bounds of the possible indices as given by the values in "
                                 "`num_boxes_per_pred_layer`.")
