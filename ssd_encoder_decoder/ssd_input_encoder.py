...
from twomartens.masterthesis.ssd_keras.ssd_encoder_decoder import matching_utils
...
class SSDInputEncoder:
    ...
    def __init__(self,
        ...
        Arguments:
            ...
            aspect_ratios_global (list, optional): The list of aspect ratios for which anchor boxes are to be
                ...
                that the resulting anchor box shapes roughly correspond to the shapes of the objects you are trying
                to detect.
            aspect_ratios_per_layer (list, optional): A list containing one aspect ratio list for each prediction layer.
                If a list is passed, it overrides `aspect_ratios_global`. Note that you should set the aspect ratios
                such
            ...
            truth box will
                be matched only to the one anchor box with the highest IoU overlap. In 'multi' mode, in addition to
                the aforementioned
                bipartite matching, all anchor boxes with an IoU overlap greater than or equal to the
                `pos_iou_threshold` will be
            ...
            normalize_coords (bool, optional): If `True`, the encoder uses relative instead of absolute coordinates.
                This means instead of using absolute tartget coordinates, the encoder will scale all coordinates to
                be within [0,1].
        ...
        if scales:
            if len(scales) != predictor_sizes.shape[0] + 1:  # Must be two nested `if` statements since
                # `list` and `bool` cannot be combined by `&`
                raise ValueError(
        ...
        else:  # If no list of scales was passed, we need to make sure that `min_scale` and `max_scale` are valid
            # values.
        ...
        if not (aspect_ratios_per_layer is None):
            if len(aspect_ratios_per_layer) != predictor_sizes.shape[0]:  # Must be two nested `if` statements
                # since `list` and `bool` cannot be combined by `&`
                raise ValueError(
                    "It must be either aspect_ratios_per_layer is None or len(aspect_ratios_per_layer) == len("
                    "predictor_sizes), but len(aspect_ratios_per_layer) == {} and len(predictor_sizes) == {}".format(
        ...
        if scales is None:
            ...
            # If a list of scales is given explicitly, we'll use that instead of computing it from `min_scale` and
            # `max_scale`.
            self.scales = scales
    ...
    def __call__(self, ground_truth_labels, diagnostics=False):
        ...
        for i in range(batch_size):  # For each batch item...
            ...
            if np.any(labels[:, [xmax]] - labels[:, [xmin]] <= 0) or np.any(labels[:, [ymax]] - labels[:, [ymin]] <= 0):
                raise DegenerateBoxError(
                    "SSDInputEncoder detected degenerate ground truth bounding boxes for batch item {} with bounding "
                    "boxes {}, ".format(
                        i, labels) +
            ...
            if self.coords == 'centroids':
                labels = bounding_box_utils.convert_coordinates(labels, start_index=xmin,
                                                                conversion='corners2centroids',
                                                                border_pixels=self.border_pixels)
            elif self.coords == 'minmax':
                labels = bounding_box_utils.convert_coordinates(labels, start_index=xmin, conversion='corners2minmax')
            ...
            similarities = bounding_box_utils.iou(labels[:, [xmin, ymin, xmax, ymax]], y_encoded[i, :, -12:-8],
            ...
            bipartite_matches = matching_utils.match_bipartite_greedy(weight_matrix=similarities)
            ...
            if self.matching_type == 'multi':
                ...
                matches = matching_utils.match_multi(weight_matrix=similarities, threshold=self.pos_iou_threshold)
        ...
        if self.coords == 'centroids':
            ...
            y_encoded[:, :, [-12, -11]] /= y_encoded[:, :, [-6, -5]] * y_encoded[:, :, [-4,
                                                                                        -3]]  # (cx(gt) - cx(anchor))
            # / w(anchor) / cx_variance, (cy(gt) - cy(anchor)) / h(anchor) / cy_variance
            y_encoded[:, :, [-10, -9]] /= y_encoded[:, :, [-6, -5]]  # w(gt) / w(anchor), h(gt) / h(anchor)
            y_encoded[:, :, [-10, -9]] = np.log(y_encoded[:, :, [-10, -9]]) / y_encoded[:, :, [-2,
                                                                                               -1]]  # ln(w(gt) / w(
            # anchor)) / w_variance, ln(h(gt) / h(anchor)) / h_variance (ln == natural logarithm)
            ...
            y_encoded[:, :, [-12, -10]] /= np.expand_dims(y_encoded[:, :, -6] - y_encoded[:, :, -8],
                                                          axis=-1)  # (xmin(gt) - xmin(anchor)) / w(anchor),
            # (xmax(gt) - xmax(anchor)) / w(anchor)
            y_encoded[:, :, [-11, -9]] /= np.expand_dims(y_encoded[:, :, -5] - y_encoded[:, :, -7],
                                                         axis=-1)  # (ymin(gt) - ymin(anchor)) / h(anchor),
            # (ymax(gt) - ymax(anchor)) / h(anchor)
            y_encoded[:, :, -12:-8] /= y_encoded[:, :, -4:]  # (gt - anchor) / size(anchor) / variance
            # for all four coordinates, where 'size' refers to w and h respectively
        elif self.coords == 'minmax':
            y_encoded[:, :, -12:-8] -= y_encoded[:, :, -8:-4]  # (gt - anchor) for all four coordinates
    ...
    def generate_anchor_boxes_for_layer(self,
        ...
        # Compute the offsets, i.e. at what pixel values the first anchor box center point will be from the top and
        ...
        boxes_tensor = bounding_box_utils.convert_coordinates(boxes_tensor, start_index=0,
        ...
        # TODO: Implement box limiting directly for `(cx, cy, w, h)` so that we don't have to unnecessarily convert
        #  back and forth.
        if self.coords == 'centroids':
