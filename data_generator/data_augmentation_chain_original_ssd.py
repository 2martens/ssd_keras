...
class SSDRandomCrop:
    ...
    def __init__(self, labels_format: Optional[Dict[str, int]] = None):
        """
        Arguments:
        ...
        self.bound_generator = validation_utils.BoundGenerator(sample_space=((None, None),
                                                                             (0.1, None),
                                                                             (0.3, None),
                                                                             (0.5, None),
                                                                             (0.7, None),
        ...
        self.patch_coord_generator = patch_sampling_ops.PatchCoordinateGenerator(must_match='h_w',
                                                                                 min_scale=0.3,
                                                                                 max_scale=1.0,
        ...
        self.image_validator = validation_utils.ImageValidator(overlap_criterion='iou',
                                                               n_boxes_min=1,
                                                               labels_format=self.labels_format,
                                                               border_pixels='half')
        ...
        self.random_crop = patch_sampling_ops.RandomPatchInf(patch_coord_generator=self.patch_coord_generator,
...
class SSDExpand:
    ...
    def __init__(self, background: Tuple[int, int, int] = (123, 117, 104),
        ...
        self.expand = patch_sampling_ops.RandomPatch(patch_coord_generator=self.patch_coord_generator,
                                                     box_filter=None,
...
class SSDPhotometricDistortions:
    ...
    def __init__(self) -> None:
        ...
        self.random_brightness = photometric_ops.RandomBrightness(lower=-32, upper=32, prob=0.5)
        self.random_contrast = photometric_ops.RandomContrast(lower=0.5, upper=1.5, prob=0.5)
        self.random_saturation = photometric_ops.RandomSaturation(lower=0.5, upper=1.5, prob=0.5)
        self.random_hue = photometric_ops.RandomHue(max_delta=18, prob=0.5)
        self.random_channel_swap = photometric_ops.RandomChannelSwap(prob=0.0)
...
class SSDDataAugmentation:
    ...
    def __init__(self,
                 img_height: int = 300,
                 img_width: int = 300,
                 background: Tuple[int, int, int] = (123, 117, 104),
                 labels_format: Optional[Dict[str, int]] = None):
        """
        Arguments:
            img_height (int): The desired height of the output images in pixels.
        ...
        self.random_flip = geometric_ops.RandomFlip(dim='horizontal', prob=0.5, labels_format=self.labels_format)
    ...
    def __call__(self, image: np.ndarray, labels: np.ndarray, return_inverter: bool = False) -> Union[
            Tuple[np.ndarray, np.ndarray], Tuple[np.ndarray, np.ndarray, np.ndarray]]:
        self.expand.labels_format = self.labels_format
        self.random_crop.labels_format = self.labels_format
