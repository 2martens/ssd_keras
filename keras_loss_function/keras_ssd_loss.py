...
def log_loss(y_true, y_pred):
    ...
    _log_loss = -tf.reduce_sum(y_true * tf.log(y_pred), axis=-1)
    return _log_loss
...
class SSDLoss:
    ...
    def compute_loss(self, y_true, y_pred):
        ...
        n_boxes = tf.shape(y_pred)[
            1]  # Output dtype: tf.int32, note that `n_boxes` in this context denotes the total number of boxes per
        # image, not the number of boxes per cell.
        ...
        # the classification
        # loss, we don't just want to know how many negative ground truth boxes there are, but for how many of those
        # there actually is
        # a positive (i.e. non-zero) loss. This is necessary because `tf.nn.top-k()` in the function below will pick
        # the top k boxes with
        # the highest losses no matter what, even if it receives a vector where all losses are zero. In the unlikely
        # event that all negative
        # classification losses ARE actually zero though, this behavior might lead to `tf.nn.top-k()` returning the
        # indices of positive
        # boxes, leading to an incorrect negative classification loss computation, and hence an incorrect overall
        # loss computation.
        ...
        # We'll keep at most `self.neg_pos_ratio` times the number of positives in `y_true`, but at least
        # `self.n_neg_min` (unless `n_neg_loses` is smaller).
        ...
        def f2():
            ...
            # belong to the background class in the ground truth data. Note that this doesn't necessarily mean that
            # the model
            ...
            _neg_class_loss = tf.reduce_sum(classification_loss * negatives_keep,
                                            axis=-1)  # Tensor of shape (batch_size,)
            return _neg_class_loss
        ...
        #    We don't compute a localization loss for negative predicted boxes (obviously: there are no ground truth
        #    boxes they would correspond to).
