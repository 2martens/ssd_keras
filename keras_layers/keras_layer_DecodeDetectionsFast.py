...
class DecodeDetectionsFast(tf.keras.layers.Layer):
    """
    ...
    def __init__(self,
        ...
        Arguments:
            ...
            `iou_threshold`
                ...
                where 'maximal' refers
                to the box score.
            top_k (int, optional): The number of highest scoring predictions to be kept for each batch item after the
                non-maximum suppression stage.
            nms_max_output_size (int, optional): The maximum number of predictions that will be left after performing
            non-maximum
                suppression.
            coords (str, optional): The box coordinate format that the model outputs. Must be 'centroids'
                i.e. the format `(cx, cy, w, h)` (box center coordinates, width, and height). Other coordinate
                formats are
                currently not supported.
            normalize_coords (bool, optional): Set to `True` if the model outputs relative coordinates (i.e.
            coordinates in [0,1])
                and you wish to transform these relative coordinates back to absolute coordinates. If the model outputs
                relative coordinates, but you do not want to convert them back to absolute coordinates, set this to
                `False`.
        ...
        if normalize_coords and ((img_height is None) or (img_width is None)):
            raise ValueError(
                "If relative box coordinates are supposed to be converted to absolute coordinates, the decoder needs "
                "the image size in order to decode the predictions, but `img_height == {}` and `img_width == {}`".
    ...
    def call(self, y_pred):
        ...
        def filter_predictions(batch_item):
            ...
            def perform_nms():
                ...
                _xmin = tf.expand_dims(predictions_conf_thresh[..., -4], axis=-1)
                _ymin = tf.expand_dims(predictions_conf_thresh[..., -3], axis=-1)
                _xmax = tf.expand_dims(predictions_conf_thresh[..., -2], axis=-1)
                _ymax = tf.expand_dims(predictions_conf_thresh[..., -1], axis=-1)
                boxes = tf.concat(values=[_ymin, _xmin, _ymax, _xmax], axis=-1)
