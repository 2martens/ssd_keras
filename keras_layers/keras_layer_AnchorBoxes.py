...
from twomartens.masterthesis.ssd_keras.bounding_box_utils.bounding_box_utils import convert_coordinates

K = tf.keras.backend


class AnchorBoxes(tf.keras.layers.Layer):
    """
    ...
    def __init__(self,
        ...
        Arguments:
            ...
            coords (str, optional): The box coordinate format to be used internally in the model (i.e. this is not
            the input format
                of the ground truth labels). Can be either 'centroids' for the format `(cx, cy, w, h)` (box center
                coordinates, width, and height),
                'corners' for the format `(xmin, ymin, xmax,  ymax)`, or 'minmax' for the format `(xmin, xmax, ymin,
                ymax)`.
        ...
        if (this_scale < 0) or (next_scale < 0) or (this_scale > 1):
            raise ValueError(
                "`this_scale` must be in [0, 1] and `next_scale` must be >0, but `this_scale` == {}, `next_scale` == "
                "{}".format(
    ...
    def call(self, x: tf.Tensor, mask=None):
        ...
        batch_size, feature_map_height, feature_map_width, feature_map_channels = tuple(x.shape.as_list())
